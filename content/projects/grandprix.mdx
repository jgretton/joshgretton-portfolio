...
status: "personal"
liveUrl: "https://grand-prix.laravel.cloud/"
...
seo:
  ...
  canonical: "https://www.joshgretton.co.uk/grandprix"
  ...
  openGraph:
    ...
    url: "https://www.joshgretton.co.uk/grandprix"
...
### Real-world constraints changing the architecture

My original plan for the tournament scoring screen was to send a POST request every time a score was entered or a round added, keeping everything server-side. It seemed clean. Then I thought harder about who would actually be using this: a coach on their phone inside a sports hall. In the UK especially, sports halls are notorious for having very poor mobile signal and incredibly weak Wi-Fi. Sending multiple requests in that environment was a recipe for lost data. The solution was to batch everything into a single request at the point of submission, rather than persisting each change to the server as it happened. This also meant rethinking the test structure and the data shape going into the backend.

### Structuring nested routes correctly
...
### React component state and useContext
...
I'd planned the scoring flow as a series of server-side requests, wrote tests around that design, and only later realised the real-world context (poor mobile signal in a sports hall) made that approach fragile. Catching it wasn't failure, but if I'd thought harder about the end user environment during planning, I'd have saved myself the rewrite.
...
One major change I want to make going forwards is involving real users earlier in the process. I tunnel vision when I build (as I am sure other developers do too), so the terminology, flow, and structure make perfect sense in my head but can feel unintuitive to someone using it for the first time. More importantly, I built this because I identified one pain point. What I should have done is sit down with the coach first and asked broader questions. There may be five other frustrations in their workflow I've never even considered, and solving those might be more valuable than anything I'd come up with alone.

## What's Next

The immediate next steps are adding localStorage to the tournament scoring flow as a persistent safety net so if the page is closed or the browser crashes mid-session, the data survives until a successful submission clears it. Beyond that, a player analytics page giving the admin a view of each player's attendance, points total, and how they're trending across the season, alongside improvements to the leaderboard to make it clearer when a score was calculated from absence rather than actually played.

A nice feature would be a multi-tenant system where other clubs can sign up, manage their own players and tournaments, and have their own public-facing leaderboard pages. Right now this is built for one club, but the underlying structure already lends itself to that kind of expansion.

## Updates

**Local Storage for Tournament Scoring**  
_12 June 2026_ · [Read full update](/grandprix/local-storage)

Added localStorage persistence for tournament status and round scoring so a lost signal or page refresh doesn't take the data with it.
